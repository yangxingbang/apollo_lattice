...
SpeedData SpeedProfileGenerator::GenerateFallbackSpeed(
  ...
  piecewise_jerk_problem.set_dddx_bound(FLAGS_fallback_longitudinal_jerk_lower_bound,
                                        FLAGS_fallback_longitudinal_jerk_upper_bound);
...
SpeedData SpeedProfileGenerator::GenerateConstantSpeedProfile(const double cruise_speed) {
  AERROR << "Generate constant speed profile";
  SpeedData speed_data;

  const double max_t = 7;
  const double unit_t = 0.1;

  speed_data.AppendSpeedPoint(0.0, 0.0, cruise_speed, 0.0, 0.0);
...
SpeedData SpeedProfileGenerator::GenerateStopProfile(const double init_speed,
  ...
  ADEBUG << "Slowdown deceleration = " << FLAGS_slowdown_profile_deceleration;
