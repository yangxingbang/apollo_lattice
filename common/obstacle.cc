...
Obstacle::Obstacle(const std::string& id,
  ...
  is_static_ = (is_static || obstacle_priority == ObstaclePriority::IGNORE ||
                speed_ < FLAGS_static_obstacle_speed_threshold);
  AINFO << "Obstacle id[" << id << "], speed = " << speed_ << ", is_static_ = "
         << is_static_ << ", is_priority_ignore = " 
         << (obstacle_priority == ObstaclePriority::IGNORE) << ", is_virtual: " << is_virtual_;
}
...
void Obstacle::CheckLaneBlocking(const ReferenceLine& reference_line) {
  ...
  //   return;
  // }
  double speed = std::hypot(Perception().velocity().x(), 
                            Perception().velocity().y());
  if (!(IsStatic() || 
      (Perception().type() == perception::PerceptionObstacle::VEHICLE &&
      speed < FLAGS_static_obstacle_speed_threshold))) {
    is_lane_blocking_ = false;
    return;
  }
