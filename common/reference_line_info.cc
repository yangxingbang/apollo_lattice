...
bool ReferenceLineInfo::Init(const std::vector<const Obstacle*>& obstacles) {
  ...
  AINFO << "adc_start_s = " << adc_sl_boundary_.start_s()
         << ", adc_end_s = " << adc_sl_boundary_.end_s();
  AINFO << "adc_start_l = " << adc_sl_boundary_.start_l()
        << ", adc_end_l = " << adc_sl_boundary_.end_l()
        << ", adc_center_l = " << (adc_sl_boundary_.start_l() 
            + adc_sl_boundary_.end_l()) / 2;
  ...
  ADEBUG << "Size of speed_bump_overlaps is: " 
        << map_path.speed_bump_overlaps().size();
...
bool ReferenceLineInfo::CombinePathAndSpeedProfile(
  ...
  for (double cur_rel_time = 0.0; cur_rel_time < speed_data_.TotalTime();
    ...
    ADEBUG << std::setprecision(9)
           << "At time = " << cur_rel_time
           << ", speed_point.s() = " << speed_point.s()
           << ", path_point_s = " << path_point.s()
...
void ReferenceLineInfo::SetTurnSignalBasedOnLaneTurnType(
  ...
  if (path_data_.path_label().find("self") != std::string::npos) {
    if (adc_sl_boundary_.end_s() < 0 ||
        adc_sl_boundary_.start_s() > reference_line_.Length()) {
      return;
    }
    auto middle_s = (adc_sl_boundary_.start_s() + adc_sl_boundary_.end_s()) / 2;
    auto middle_l = (adc_sl_boundary_.start_l() + adc_sl_boundary_.end_l()) / 2;
    double lane_left_width = 0.0;
    double lane_right_width = 0.0;
    reference_line_.GetLaneWidth(middle_s, &lane_left_width, &lane_right_width);

    bool is_lane_blocking = false;
    for (const auto* obstacle : path_decision_.obstacles().Items()) {
      if (obstacle->IsLaneBlocking() && !obstacle->IsVirtual() &&
          obstacle->PerceptionSLBoundary().end_s() > adc_sl_boundary_.start_s()) {
        is_lane_blocking = true;
        ADEBUG << "lane blocking obstacle id is " << obstacle->Id();
        break;
      }
    }

    if (middle_l >= lane_left_width && !is_lane_blocking) {
      vehicle_signal->set_turn_signal(VehicleSignal::TURN_RIGHT);
      return;
    } else if (middle_l <= -lane_right_width && !is_lane_blocking) {
      vehicle_signal->set_turn_signal(VehicleSignal::TURN_LEFT);
      return;
    }
  }
...
void ReferenceLineInfo::SetEmergencyLightOff() {
  vehicle_signal_.set_emergency_light(false);
}
...
void ReferenceLineInfo::ExportVehicleSignal(
  ...
  *vehicle_signal = vehicle_signal_;
  ...
  ADEBUG << "vehicle turn signal is: " << vehicle_signal->turn_signal();
}
...
void ReferenceLineInfo::ExportDecision(DecisionResult* decision_result) const {
  ...
  // Only for HMI speed-limit display
  auto* target_lane = main_decision->add_target_lane();
  const double current_s = adc_planning_point_.path_point().s();
  const double speed_limit = reference_line_.GetSpeedLimitFromS(current_s);
  ADEBUG << "For HMI, at s = " << current_s << ", speed_limit = " << speed_limit;
  target_lane->set_speed_limit(speed_limit);
