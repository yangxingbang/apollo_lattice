...
DEFINE_string(truck_config_path,
             "/apollo/modules/planning/conf/"
              "truck_param.pb.txt",
              "truck params config file");
...
DEFINE_bool(enable_map_speed_limit, true,
            "Enable map speed linit in planning.");
...
DEFINE_double(fallback_longitudinal_jerk_lower_bound, -10.0,
              "The lower bound of longitudinal jerk for speed fallback.");
DEFINE_double(fallback_longitudinal_jerk_upper_bound, 2.0,
              "The upper bound of longitudinal jerk for speed fallback.");
...
DEFINE_double(crosswalks_speed_limit, 6.0,
              "the speed limit when passing a crosswalk, m/s. The default "
              "speed limit is 6.0 m/s.");
...
DEFINE_bool(enable_use_fake_obstacle_for_demo, false,
            "add fake obstacles for demo");
...
DEFINE_bool(enable_always_stop_pedestrian, false,
            "always stop if there are pedestrian obstacles in specific area.");
...
// parameters for minimize overhang path optimizer
DEFINE_double(gradually_discreted_splice, 50.0,
              "The splice length of gradually discretization in longitudinal direction.");

DEFINE_double(max_heading_error, M_PI_2,
             "The max heading error of Truck Head difference to Referece Point.");

DEFINE_double(max_trailer_heading_error, M_PI_2,
             "The max heading error of Truck Trailer difference to Truck Head.");
...
DEFINE_double(default_cruise_speed, 10.0, "default cruise speed");
...
DEFINE_bool(use_front_axe_center_in_path_planning, false,
            ...
            "more agile.");
DEFINE_bool(print_path_bounder,false,"print the path bounder in path bounder decider.");
DEFINE_bool(print_log_parse_speed,false,"print the log parse speed.");
DEFINE_bool(print_log_parse_reference_line,false,"print the log parse reference line.");
