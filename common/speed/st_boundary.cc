...
STBoundary::STBoundary(
  ...
  AINFO << "is_accurate_boundary: " << is_accurate_boundary;
...
STBoundary STBoundary::CreateInstance(
  ...
  if (lower_points.size() != upper_points.size() || lower_points.size() < 2) {
    AINFO << "size < 2 in CreateInstance";
    return STBoundary();
  }

  std::vector<std::pair<STPoint, STPoint>> point_pairs;
  AINFO << "i in CreateInstance: ";
...
STBoundary STBoundary::CreateInstanceAccurate(
    const std::vector<STPoint>& lower_points,
    const std::vector<STPoint>& upper_points) {
      AINFO << "lower_points.size() < 2 in CreateInstanceAccurate";
  if (lower_points.size() != upper_points.size() || lower_points.size() < 2) {
    return STBoundary();
  }

  std::vector<std::pair<STPoint, STPoint>> point_pairs;
  AINFO << "i in CreateInstanceAccurate: ";
  for (size_t i = 0; i < lower_points.size(); ++i) {
    AINFO << "i: " << i;
    point_pairs.emplace_back(
        STPoint(lower_points.at(i).s(), lower_points.at(i).t()),
...
STBoundary STBoundary::CutOffByT(const double t) const {
  ...
  AINFO << "lower_points_.size(): " << lower_points_.size();
...
/*used in lattice planner*/
bool STBoundary::MaxMinSTofUpperLowerPoints(const std::vector<STPoint>& lower_points, 
                                            const std::vector<STPoint>& upper_points) {
  ...
  for (const auto& point : upper_points) {
    max_s_ = std::fmax(max_s_, point.s());
  }
  min_t_ = lower_points_.front().t();
  max_t_ = lower_points_.back().t();

  AINFO << "min_s_: " << min_s_ << ", min_t_: " << min_t_
        << ", max_s_: " << max_s_ << ", max_t_: " << max_t_;
