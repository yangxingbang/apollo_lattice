...
bool CollisionChecker::InCollision(
  ...
  for (size_t i = 0; i < discretized_trajectory.NumOfPoints(); ++i) {
    ...
    for (const auto& obstacle_box : predicted_bounding_rectangles_[i]) {
      if (ego_box.HasOverlap(obstacle_box)) {
        AINFO << "ego first:" << ego_box.GetAllCorners()[0].x() << ", "
              << ego_box.GetAllCorners()[0].y();
        AINFO << "ego second:" << ego_box.GetAllCorners()[1].x() << ", "
              << ego_box.GetAllCorners()[1].y();
        AINFO << "ego third:" << ego_box.GetAllCorners()[2].x() << ", "
              << ego_box.GetAllCorners()[2].y();
        AINFO << "ego fourth:" << ego_box.GetAllCorners()[3].x() << ", "
              << ego_box.GetAllCorners()[3].y();

        AINFO << "obs first:" << obstacle_box.GetAllCorners()[0].x() << ", "
              << obstacle_box.GetAllCorners()[0].y();
        AINFO << "obs second:" << obstacle_box.GetAllCorners()[1].x() << ", "
              << obstacle_box.GetAllCorners()[1].y();
        AINFO << "obs third:" << obstacle_box.GetAllCorners()[2].x() << ", "
              << obstacle_box.GetAllCorners()[2].y();
        AINFO << "obs fourth:" << obstacle_box.GetAllCorners()[3].x() << ", "
              << obstacle_box.GetAllCorners()[3].y();
        return true;
      }
    }
  }
