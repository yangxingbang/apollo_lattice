...
Status PathLaneBorrowDecider::Process(
  ...
  if (Decider::config_.path_lane_borrow_decider_config()
    ...
    ADEBUG << "is_path_lane_borrow: true";
  }
  return Status::OK();
}
...
bool PathLaneBorrowDecider::IsNecessaryToBorrowLane(
  ...
  if (mutable_path_decider_status->is_in_path_lane_borrow_scenario()) {
    ...
    if (mutable_path_decider_status->able_to_use_self_lane_counter() >= 46) {
    ...
    AINFO << "path_decider_status.decided_side_pass_direction().empty(): " 
          << path_decider_status.decided_side_pass_direction().empty();
    if (path_decider_status.decided_side_pass_direction().empty()) {
      ...
      if (!left_borrowable && !right_borrowable) {
        ...
        // 20220719, only the right-side two lanes is allowable for truck
        if (right_borrowable) {
          mutable_path_decider_status->add_decided_side_pass_direction(
              PathDeciderStatus::RIGHT_BORROW);
        } else {
