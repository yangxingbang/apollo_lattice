...
Status PathBoundsDecider::Process(
  ...
  Reference_line_info = frame->mutable_reference_line_info();
  FLAGS_replan_lateral_distance_threshold = lat_offset;
...
void PathBoundsDecider::InitPathBoundsDecider(
  ...
  AINFO << "adc_frenet_s_ = " << adc_frenet_s_
        << ", adc_frenet_l_ = " << adc_frenet_l_
        << ", adc_offset_to_map = " << offset_to_map
        << ", adc_l_to_lane_center_ =  " << adc_l_to_lane_center_;
  AINFO << "adc_frenet_sd_ = " << adc_frenet_sd_;

  common::TrajectoryPoint trailer_point =
      TractorTrailerModel::CalculateTrailerCoordinate(planning_start_point);
  auto adc_trailer_sl_info = reference_line.ToFrenetFrame(trailer_point);
  adc_trailer_l_to_lane_center_ = adc_trailer_sl_info.second[0] + offset_to_map;
...
Status PathBoundsDecider::GenerateRegularPathBound(
  ...
  if (!GetBoundaryFromLanesAndADC(reference_line_info, lane_borrow_info, 1.5,
...
int PathBoundsDecider::IsPointWithinPathBound(
  ...
  // Let idx_after = 1 to fix the bug that ADC can't stop at the end of
  // EMERGENCY_PULL_OVER_STANDBY stage. wsy1203
  int idx_after = 1;
  ...
  if (std::get<1>(path_bound[idx_before]) <= point_sl.l()+0.85 &&
      std::get<2>(path_bound[idx_before]) >= point_sl.l()+0.85 &&
      std::get<1>(path_bound[idx_after]) <= point_sl.l()+0.85 &&
      std::get<2>(path_bound[idx_after]) >= point_sl.l()+0.85) {
    return idx_after;
  }
  ...
  while ((search_backward && idx >= 0 &&
    ...
    if (is_feasible_window) {
      ...
      const double pull_over_l =
          std::get<1>(pull_over_point) + pull_over_space_width / 2.0 -0.85;
...
// /*
    ...
    double curr_road_left_width = 0.0;
    double curr_road_right_width = 0.0;
    const double road_width_limit = 10.0;
    const double turn_inside_buffer = 0.0;
    reference_line.GetRoadWidth(curr_s, &curr_road_left_width,
                                     &curr_road_right_width);
// add a max boundary by ctx 0907
    curr_road_left_width = std::fmin(curr_road_left_width,3.75*2);
    curr_road_right_width = std::fmin(curr_lane_left_width,3.75*2);
    const double trailer_length =
        TruckConfigHelper::GetConfig().trailer_param().wheel_base() +
        TruckConfigHelper::GetConfig().trailer_param().rear_overhang() + 10.0;
    const auto& turn_type_tractor = reference_line_info.GetPathTurnType(curr_s);
    const auto& turn_type_trailer = reference_line_info.GetPathTurnType(curr_s - trailer_length);
    auto turn_type = hdmap::Lane::NO_TURN;
    if (turn_type_tractor == hdmap::Lane::RIGHT_TURN ||
        turn_type_trailer == hdmap::Lane::RIGHT_TURN) {
      turn_type = hdmap::Lane::RIGHT_TURN;
    } else if (turn_type_tractor == hdmap::Lane::LEFT_TURN ||
        turn_type_trailer == hdmap::Lane::LEFT_TURN) {
    ...
    if (turn_type == hdmap::Lane::RIGHT_TURN) {
      curr_left_bound_lane = std::fmax(curr_left_bound_lane,
                      std::fmin(curr_road_left_width, road_width_limit));
    } else if (turn_type == hdmap::Lane::LEFT_TURN) {
      curr_left_bound_lane =
                  std::fmax(curr_left_bound_lane - turn_inside_buffer, 0);
    }

    // double curr_left_bound_adc =
    //     std::fmax(adc_l_to_lane_center_,
    //               std::fmax(adc_l_to_lane_center_ + ADC_speed_buffer,
    //                         adc_trailer_l_to_lane_center_)) +
    //     GetBufferBetweenADCCenterAndEdge() + ADC_buffer;
    double curr_left_bound_adc =
        std::fmax(adc_l_to_lane_center_,
                  adc_l_to_lane_center_ + ADC_speed_buffer) +
        GetBufferBetweenADCCenterAndEdge() + ADC_buffer;
    if (turn_type == hdmap::Lane::RIGHT_TURN/* || turn_type == hdmap::Lane::LEFT_TURN*/) {
    // change the diff
      AINFO << "size of references is " << Reference_line_info->size();
      if(Reference_line_info->size() == 1){
        FLAGS_replan_lateral_distance_threshold = 2.50;
      }
      AINFO << "FLAGS_replan_lateral_distance_threshold " << FLAGS_replan_lateral_distance_threshold;
       curr_left_bound_adc =
        std::fmax(adc_l_to_lane_center_,
                  std::fmax(adc_l_to_lane_center_ + ADC_speed_buffer,
                            adc_trailer_l_to_lane_center_ + 2.0)) +
        GetBufferBetweenADCCenterAndEdge() + ADC_buffer;
    }

       // modify the boundary by ctx 0907
    //     if (turn_type == hdmap::Lane::RIGHT_TURN) {
    //    curr_left_bound_adc =
    //     std::fmax(adc_l_to_lane_center_,
    //               std::fmax(adc_l_to_lane_center_ + ADC_speed_buffer,
    ...
    if (turn_type == hdmap::Lane::LEFT_TURN) {
      curr_right_bound_lane = std::fmin(curr_right_bound_lane,
                      -std::fmin(curr_road_right_width, road_width_limit));
    } else if (turn_type == hdmap::Lane::RIGHT_TURN) {
      curr_right_bound_lane =
                  std::fmin(curr_right_bound_lane + turn_inside_buffer, 0);
    }

    // double curr_right_bound_adc =
    //     std::fmin(adc_l_to_lane_center_,
    //               std::fmin(adc_l_to_lane_center_ + ADC_speed_buffer,
    //                         adc_trailer_l_to_lane_center_)) -
    //     GetBufferBetweenADCCenterAndEdge() - ADC_buffer;
    double curr_right_bound_adc =
        std::fmin(adc_l_to_lane_center_,
    ...
    // modify the bboundary by ctx 0907
    //   if (turn_type == hdmap::Lane::LEFT_TURN) {
    //   curr_right_bound_adc =
    //     std::fmin(adc_l_to_lane_center_,
    //               std::fmin(adc_l_to_lane_center_ + ADC_speed_buffer,
   ...
   if(FLAGS_print_path_bounder){
    ADEBUG << "At path_bound[" << i << "], curr_s = " << curr_s
           << ", lane_left_width = " << curr_lane_left_width
           << ", lane_right_width = " << curr_lane_right_width
           << ", road_left_width = " << curr_road_left_width
           << ", road_right_width = " << curr_road_right_width
           << ", left_bound = " << curr_left_bound
           << ", right_bound = " << curr_right_bound;
   }
// */

    //  fallback the origin boundary for test by ctx 0907
    /*
     static constexpr double kMaxLateralAccelerations = 1.5;
    double offset_to_map = 0.0;
    reference_line.GetOffsetToMap(curr_s, &offset_to_map);
...
void PathBoundsDecider::GetBoundaryFromLaneChangeForbiddenZone(
  ...
  for (size_t i = 0; i < path_bound->size(); ++i) {
    ...
    // +/- 0.5 to make ADC stay at the center of current lane
    std::get<1>((*path_bound)[i]) =
        adc_frenet_l_ > curr_lane_left_width
            ? curr_lane_left_width + GetBufferBetweenADCCenterAndEdge() + 0.5
            : std::get<1>((*path_bound)[i]);
    std::get<1>((*path_bound)[i]) =
        std::fmin(std::get<1>((*path_bound)[i]), adc_frenet_l_ - 0.1);
    std::get<2>((*path_bound)[i]) =
        adc_frenet_l_ < -curr_lane_right_width
            ? -curr_lane_right_width - GetBufferBetweenADCCenterAndEdge() - 0.5
            : std::get<2>((*path_bound)[i]);
    std::get<2>((*path_bound)[i]) =
...
bool PathBoundsDecider::GetBoundaryFromStaticObstacles(
  ...
  for (size_t i = 0; i < sorted_obstacles.size(); ++i) {
    ADEBUG << "sorted_obstacles[" << i
           << "]: is_start_s, s, l_min, l_max, obstacle_id = "
           << std::get<0>(sorted_obstacles[i]) << ", "
           << std::get<1>(sorted_obstacles[i]) << ", "
           << std::get<2>(sorted_obstacles[i]) << ", "
           << std::get<3>(sorted_obstacles[i]) << ", "
           << std::get<4>(sorted_obstacles[i]);
  }
  ...
  for (size_t i = 1; i < path_boundaries->size(); ++i) {
    ...
    if (obs_idx < sorted_obstacles.size() &&
      ...
      // If there is no obstacle but still in path lane borrow scenario, do not go
      // back to ego lane immediately. The go back time is determined by
      // able_to_use_self_lane_counter.
      // if ((sorted_obstacles.empty()
      //     || (!sorted_obstacles.empty() && std::get<1>(sorted_obstacles.back()) < curr_s))
      //     && mutable_path_decider_status->is_in_path_lane_borrow_scenario()) {
      //   ADEBUG << "Adjust path boundary for no obstacle but still in path lane borrow scenario";
      //   if (adc_frenet_l_ > 0.1) {
      //     std::get<1>((*path_boundaries)[i]) = adc_frenet_l_ - 0.1;
      //   } else if (adc_frenet_l_ < -0.1) {
      //     std::get<2>((*path_boundaries)[i]) = adc_frenet_l_ + 0.1;
      //   }
      // }
...
std::vector<ObstacleEdge> PathBoundsDecider::SortObstaclesForSweepLine(
  ...
  for (const auto* obstacle : indexed_obstacles.Items()) {
    ...
    if (obstacle->PerceptionSLBoundary().end_s() + FLAGS_obstacle_lon_end_buffer < adc_frenet_s_) {
      continue;
    }
    if (obstacle->PerceptionSLBoundary().end_l() < -6 ||
        obstacle->PerceptionSLBoundary().start_l() > 6) {
      continue;
    }
    // Decompose each obstacle's rectangle into two edges: one at
    // start_s; the other at end_s.
    const auto obstacle_sl = obstacle->PerceptionSLBoundary();
    ADEBUG << "SortObstacles, obstacle ID: " << obstacle->Id()
           << ", start_s: " << obstacle_sl.start_s()
           << ", end_s: " << obstacle_sl.end_s()
           << ", start_l: " << obstacle_sl.start_l()
           << ", end_l: " << obstacle_sl.end_l();
    double adjusted_start_l = obstacle_sl.start_l();
    double adjusted_end_l = obstacle_sl.end_l();
    if (obstacle_sl.start_l() > -1.9 && obstacle_sl.start_l() < 0) {
      adjusted_start_l = -1.9;
    } else if (obstacle_sl.start_l() >= 0 && obstacle_sl.start_l() < 1.9) {
      adjusted_start_l = 0;
    }
    if (obstacle_sl.end_l() > -1.9 && obstacle_sl.end_l() < 0) {
      adjusted_end_l = 0;
    } else if (obstacle_sl.end_l() >= 0 && obstacle_sl.end_l() < 1.9) {
      adjusted_end_l = 1.9;
    }
    ...
    // sorted_obstacles.emplace_back(
    //     1, obstacle_sl.start_s() - FLAGS_obstacle_lon_start_buffer,
    //     adjusted_start_l - FLAGS_obstacle_lat_buffer,
    //     adjusted_end_l + FLAGS_obstacle_lat_buffer, obstacle->Id());
    // sorted_obstacles.emplace_back(
    //     0, obstacle_sl.end_s() + FLAGS_obstacle_lon_end_buffer,
    //     adjusted_start_l - FLAGS_obstacle_lat_buffer,
    //     adjusted_end_l + FLAGS_obstacle_lat_buffer, obstacle->Id());
    ...
    if (obstacle->IsStatic() && obstacle->speed() < 0.5) {
      ...
      sorted_obstacles.emplace_back(
        ...
        adjusted_start_l - FLAGS_obstacle_lat_buffer,
        adjusted_end_l + FLAGS_obstacle_lat_buffer, obstacle->Id());
      ADEBUG << "obstacle is static, bound_start = "
             << obstacle_sl.start_s() - FLAGS_obstacle_lon_start_buffer
             << ", bound_end = "
             << obstacle_sl.end_s() + FLAGS_obstacle_lon_end_buffer;
    } else {
      const auto& obstacle_theta = obstacle->Perception().theta();
      ADEBUG << "obstacle_theta = " << obstacle_theta << ", adc_theta_ = " << adc_theta_;
      const double heading_diff = common::math::NormalizeAngle(common::math::AngleDiff(obstacle_theta, adc_theta_));
      ADEBUG << "heading_diff = " << heading_diff;

      double bound_start_s = obstacle_sl.start_s();
      double bound_end_s = obstacle_sl.end_s();
      if (std::fabs(heading_diff) > M_PI / 2) {
        double adc_length = VehicleConfigHelper::GetConfig().vehicle_param().length();
        double adc_s = adc_frenet_s_ + adc_length;
        double collision_time = std::max(0.0,
            (obstacle_sl.start_s() - adc_s) / (obstacle->speed() + adc_speed_));
        bound_start_s = obstacle_sl.start_s() -
           obstacle->speed() * collision_time - FLAGS_obstacle_lon_start_buffer;
           ADEBUG << "adc_s = " << adc_s << ", obstacle_sl.start_s() = " << obstacle_sl.start_s()
                  << ", bound_start_s = " << bound_start_s;
        bound_end_s =  obstacle_sl.end_s() + FLAGS_obstacle_lon_end_buffer;
      } else {
        bound_start_s = obstacle_sl.start_s() - FLAGS_obstacle_lon_start_buffer;
        bound_end_s =  obstacle_sl.end_s() + FLAGS_obstacle_lon_end_buffer +
                       obstacle->speed() * kPreviewTimeNonStaticObstacle;
      }
      sorted_obstacles.emplace_back(
        1, bound_start_s,
        adjusted_start_l - FLAGS_obstacle_lat_buffer,
      ...
      sorted_obstacles.emplace_back(
        ...
        adjusted_start_l - FLAGS_obstacle_lat_buffer,
        adjusted_end_l + FLAGS_obstacle_lat_buffer, obstacle->Id());
      ADEBUG << "obstacle is not static, bound_start_s = "
             << bound_start_s << ", bound_end_s = "  << bound_end_s;
    }
  }
...
bool PathBoundsDecider::UpdatePathBoundaryAndCenterLine(
  ...
  if (new_l_min > new_l_max) {
    ...
    ADEBUG << "new_l_min = " << new_l_min << ", new_l_max = " << new_l_max;
    return false;
  }
