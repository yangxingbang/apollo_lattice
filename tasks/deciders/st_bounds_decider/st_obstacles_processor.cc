...
Status STObstaclesProcessor::MapObstaclesToSTBoundaries(
  ...
  for (const auto* obs_item_ptr : path_decision->obstacles().Items()) {
    ...
    AINFO << "lower_points.size() in MapObstaclesToSTBoundaries: " << lower_points.size();

    for (std::size_t i = 0; i < lower_points.size(); i++) {
      ADEBUG << "lower_points[" << i << "]: s = " << lower_points[i].s() 
            << ", t = " << lower_points[i].t();
      ADEBUG << "upper_points[" << i << "]: s = " << upper_points[i].s() 
            << ", t = " << upper_points[i].t();
    }
...
bool STObstaclesProcessor::ComputeObstacleSTBoundary(
  ...
  *is_caution_obstacle = false;
  ...
  if (obs_trajectory.trajectory_point().empty()) {
    ...
    if (GetOverlappingS(adc_path_points, obs_box, kADCSafetyLBufferStatic,
                        &overlapping_s)) {
    ...
    *obs_caution_end_t = planning_time_;
    ...
    for (const auto& obs_traj_pt : obs_trajectory.trajectory_point()) {
      ...
      Box2d obs_box = obstacle.GetBoundingBox(obs_traj_pt);
      obs_box.LongitudinalExtend(2.0 * FLAGS_lon_collision_buffer);
      ADEBUG << obs_box.DebugString();
      std::pair<double, double> overlapping_s;
      if (GetOverlappingS(adc_path_points, obs_box, kADCSafetyLBufferDynamic,
                          &overlapping_s)) {
