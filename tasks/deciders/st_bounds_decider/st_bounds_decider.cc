...
void STBoundsDecider::InitSTBoundsDecider(
  ...
  const double desired_speed = reference_line_info->GetCruiseSpeed();
  st_guide_line_.Init(desired_speed*0.5);
  const double max_acc = 2.5;
  const double max_dec = 5.0;
  const double max_v = desired_speed * 1.5;
