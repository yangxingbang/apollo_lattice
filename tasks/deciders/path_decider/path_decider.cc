...
bool PathDecider::MakeStaticObstacleDecision(
  ...
  for (const auto *obstacle : path_decision->obstacles().Items()) {
    ...
    // - add STOP decision for pedestrian
    const double kPedestrianStopLateralDistance = 8.0;
    if (obstacle_type_name == "PEDESTRIAN" 
        && FLAGS_enable_always_stop_pedestrian) {
      ADEBUG << "obstacle_id[" << obstacle_id << "] is PEDESTRIAN";
      const double path_length = frenet_path.back().s() - frenet_path.front().s();
      const double check_length = std::min(path_length, 80.0);

      double pedestrian_stop_limit = kPedestrianStopLateralDistance * 
                        (sl_boundary.start_s() - frenet_path.front().s()) /
                        check_length;
      pedestrian_stop_limit = std::min(pedestrian_stop_limit, 
                                       kPedestrianStopLateralDistance);
      pedestrian_stop_limit = std::max(pedestrian_stop_limit, half_width + 0.0);
      if (sl_boundary.end_l() > curr_l - pedestrian_stop_limit &&
          sl_boundary.start_l() < curr_l + pedestrian_stop_limit) {
        ADEBUG << "Add stop decision for pedestrian, id: " << obstacle_id
               << ", start_s: " << sl_boundary.start_s() << ", end_s: "
               << sl_boundary.end_s() << ", start_l: " << sl_boundary.start_l()
               << ", end_l: " << sl_boundary.end_l();
        ObjectDecisionType object_decision;
        *object_decision.mutable_stop() = GenerateObjectStopDecision(*obstacle);
        path_decision->AddLongitudinalDecision("PathDecider/stop_for_pedestrian",
                                             obstacle->Id(), object_decision);
      }
    }
...
ObjectStop PathDecider::GenerateObjectStopDecision(
  ...
  ADEBUG << "stop_distance = " << stop_distance << ", obstacle_start_s = "
      << obstacle.PerceptionSLBoundary().start_s() << ", stop_ref_s = "
      << stop_ref_s;
