...
bool SpeedDecider::CreateFollowDecision(
  ...
  ADEBUG << "FOLLOW: obstacle_id[" << obstacle.Id() << "] obstacle_type["
         << PerceptionObstacle_Type_Name(obstacle_type) << "] obstacle_velocity: "
         << std::sqrt(obstacle.Perception().velocity().x() * obstacle.Perception().velocity().x()
         + obstacle.Perception().velocity().y() * obstacle.Perception().velocity().y());
