...
Status LaneChangeDecider::Process(
  ...
  if (!has_change_lane) {
    ...
    if (prev_status->status() == ChangeLaneStatus::IN_CHANGE_LANE) {
      ADEBUG << "ChangeLaneStatus: IN_CHANGE_LANE";
      ...
      ADEBUG << "ChangeLaneStatus: CHANGE_LANE_FAILED";
      ...
      ADEBUG << "ChangeLaneStatus: CHANGE_LANE_FINISHED";  
      if (now - prev_status->timestamp() <
      ...
      ADEBUG << "ChangeLaneStatus: Unknown state";
      const std::string msg =
          absl::StrCat("Unknown state: ", prev_status->ShortDebugString());
      AERROR << msg;
      return Status(ErrorCode::PLANNING_ERROR, msg);
...
void LaneChangeDecider::PrioritizeChangeLane(
  ...
  std::list<ReferenceLineInfo> new_reference_line_info;
  ...
  while (iter != reference_line_info->end()) {
    ...
    if ((is_prioritize_change_lane && iter->IsChangeLanePath()) ||
      ...
      ADEBUG << "iter->IsChangeLanePath(): " << iter->IsChangeLanePath();
      ReferenceLineInfo temp = *iter;
      new_reference_line_info.push_back(temp);
      break;
    }
    ++iter;
  }
  auto it = reference_line_info->begin();
  while (it != reference_line_info->end()) {
    if (it != iter) {
      ReferenceLineInfo temp = *it;
      new_reference_line_info.push_back(temp);
    }
    ++it;
  }
  *reference_line_info = new_reference_line_info;
