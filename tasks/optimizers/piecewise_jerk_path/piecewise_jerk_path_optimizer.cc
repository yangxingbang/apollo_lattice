...
bool PiecewiseJerkPathOptimizer::OptimizePath(
  ...
  // wsy1126
  ADEBUG << "init_state = (" << init_state[0] << ", " << init_state[1] <<
      ", " << init_state[2] << ")";
  ADEBUG << "end_state = (" << end_state[0] << ", " << end_state[1] <<
      ", " << end_state[2] << ")";
  ADEBUG << "weight_x = " << w[0] << ", weight_dx = " << w[1] <<
      ", weight_ddx = " << w[2] << ", weight_dddx = " << w[3];
  for (std::size_t i = 0; i < lat_boundaries.size(); i++) {
    ADEBUG << "For s[" << i * delta_s << "], x_bounds = [" 
        << lat_boundaries[i].first << ", " << lat_boundaries[i].second
        << "], dx_bounds = [" << -FLAGS_lateral_derivative_bound_default 
        << ", " << FLAGS_lateral_derivative_bound_default
        << "] , ddx_bounds = [" << ddl_bounds[i].first << ", "
        << ddl_bounds[i].second << "], dddx_bounds = [" << -jerk_bound
        << ", " << jerk_bound << "]";
  }
