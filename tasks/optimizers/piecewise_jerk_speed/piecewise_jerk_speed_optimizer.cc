...
Status PiecewiseJerkSpeedOptimizer::Process(const PathData& path_data,
  ...
  if (st_graph_data.init_point().v() < 1e-6 &&
      st_graph_data.init_point().a() < 0.0) {
    init_a = 0.0;
  }
  ...
  std::vector<std::pair<double, double>> s_bounds_log;
  ...
  std::vector<std::pair<double, double>> s_dot_bounds_log;
  ...
  s_dot_bounds_log = s_dot_bounds;
  piecewise_jerk_problem.set_dx_bounds(std::move(s_dot_bounds));

  // wsy1122
  ADEBUG << "init_s = " << init_s[0] << ", init_v = " << init_s[1]
         << ", init_a = " << init_s[2];
  ADEBUG << "ddx_weight = " << piecewise_jerk_speed_config.acc_weight() << ", "
         << "dddx_weight = " << piecewise_jerk_speed_config.jerk_weight() << ", "
         << "kappa_penalty_weight = " <<  piecewise_jerk_speed_config.kappa_penalty_weight() << ", "
         << "ref_x_weight = " << piecewise_jerk_speed_config.ref_s_weight() << ","
         << "ref_dx_weight = " << piecewise_jerk_speed_config.ref_v_weight();
  ADEBUG << "dx_ref_ = " << reference_line_info_->GetCruiseSpeed();
  ADEBUG << "dddx_bound_ = [" << FLAGS_longitudinal_jerk_lower_bound << ", " 
                              << FLAGS_longitudinal_jerk_upper_bound << "]";
  if(FLAGS_print_log_parse_speed){
    for (int i = 0; i < num_of_knots; ++i) {
      AINFO << "log-parse::QP bound(t,s_bounds,ds_bounds,dds_bounds,s_ref,penalty_ds): "  << std::fixed << std::setprecision(2) << i * delta_t  //t = 
            << "; " << s_bounds_log[i].first << " , " << s_bounds_log[i].second  //x_bounds =  
            << "; " << s_dot_bounds_log[i].first << " , " << s_dot_bounds_log[i].second  //dx_bounds = 
            << "; " << veh_param.max_deceleration() << " , " << veh_param.max_acceleration()  //ddx_bounds = 
            << "; " << x_ref[i]  //x_ref = 
            << "; " <<  penalty_dx[i];  //penalty_dx = 
    }
  }
  ...
  if(FLAGS_print_log_parse_speed){
    for (int i = 0; i < num_of_knots; ++i) {
      AINFO << "log-parse::QP result(t,s,ds,dds): "  << std::fixed << std::setprecision(2) << i * delta_t << " , " << s[i] << " , " << ds[i]
            << " , " << dds[i];
    }
  }
  speed_data->clear();
