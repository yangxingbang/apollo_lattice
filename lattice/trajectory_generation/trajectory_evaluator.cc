...
TrajectoryEvaluator::TrajectoryEvaluator(
  ...
  AINFO << "lon traj size: " << lon_trajectories.size();
  for (const auto& lon_trajectory : lon_trajectories) {
    double lon_end_s = lon_trajectory->Evaluate(0, end_time);
    // AINFO << "end_time: " << end_time << ", lon_end_s: " << lon_end_s;
    // represents traj pass through the stop point, that is unreasonable.
    ...
    AINFO << "lat traj size: " << lat_trajectories.size();
...
// get the smallest cost traj, then pop it out
...
double TrajectoryEvaluator::LonCollisionCost(
  ...
  for (size_t i = 0; i < path_time_intervals_.size(); ++i) {
    ...
    for (const auto& m : pt_interval) {
      double dist = 0.0;
      // first is the obj's s_lower, second is the obj's s_upper
      // at each time step, ego car's location is behind of obj
      if (traj_s < m.first - FLAGS_lon_collision_yield_buffer) {
        // dist between ego car and obj
        dist = m.first - FLAGS_lon_collision_yield_buffer - traj_s;
        // at each time step, ego car's location is ahead of obj
      } else if (traj_s > m.second + FLAGS_lon_collision_overtake_buffer) {
        dist = traj_s - m.second - FLAGS_lon_collision_overtake_buffer;
      }
      // the better, the dist is the proper longer 
...
std::vector<double> TrajectoryEvaluator::ComputeLongitudinalGuideVelocity(
  ...
  if (!planning_target.has_stop_point()) {
    ...
    for (double t = 0.0; t < FLAGS_trajectory_time_length;
         t += FLAGS_trajectory_time_resolution) {
      // AINFO << "reference_s_dot: " << lon_traj.Evaluate(1, t);
      reference_s_dot.emplace_back(lon_traj.Evaluate(1, t));
    }
  } else {
    AINFO << "has stop point!";
    ...
    AINFO << "target stop s: " << planning_target.stop_point().s() 
          << ", init_s: " << init_s_[0] << ", target cruise speed: "
          << planning_target.cruise_speed() << ", init_v: "
          << init_s_[1] << ", a_comfort: " << a_comfort << ", d_comfort: "
          << d_comfort;
    ...
    for (double t = 0.0; t < FLAGS_trajectory_time_length;
         t += FLAGS_trajectory_time_resolution) {
      // AINFO << "reference_s_dot: " << lon_ref_trajectory->Evaluate(1, t);
