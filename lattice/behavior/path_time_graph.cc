...
void PathTimeGraph::SetStaticObstacle(
  ...
  AINFO << "before out of range: " << obstacle_id
         << ", start_s : " << sl_boundary.start_s() << ", range_second: " << path_range_.second
         << ", end_s : " << sl_boundary.end_s() << ", range_first: " << path_range_.first
         << ", start_l : " << sl_boundary.start_l() << ", left_width: " << left_width
         << ", end_l : " << sl_boundary.end_l() << ", -right_width: " << -right_width;

  // obs is not considered in planning, and not in path time graph, because obs is out of range.
  if (sl_boundary.start_s() > path_range_.second ||
    ...
    AINFO << "Obstacle [" << obstacle_id << "] is out of range.";
    return;
  }

  path_time_obstacle_map_[obstacle_id].set_id(obstacle_id);
  AINFO << "obs map id: " << path_time_obstacle_map_[obstacle_id].id();
  // error here
  path_time_obstacle_map_[obstacle_id].set_bottom_left_point(
      SetPathTimePoint(obstacle_id, sl_boundary.start_s(), 0.0));
  // AINFO << "obs map bottom left: " << path_time_obstacle_map_[obstacle_id].bottom_left_point().s()
  //                        << ", t:" << path_time_obstacle_map_[obstacle_id].bottom_left_point().t();
  ...
  // AINFO << "obs map bottom right: " << path_time_obstacle_map_[obstacle_id].bottom_right_point().s()
  //                         << ", t:" << path_time_obstacle_map_[obstacle_id].bottom_right_point().t();
  path_time_obstacle_map_[obstacle_id].set_upper_left_point(
      SetPathTimePoint(obstacle_id, sl_boundary.end_s(), 0.0));
  // AINFO << "obs map upper left: " << path_time_obstacle_map_[obstacle_id].upper_left_point().s()
  //                       << ", t:" << path_time_obstacle_map_[obstacle_id].upper_left_point().t();
  ...
  // AINFO << "obs map upper right: " << path_time_obstacle_map_[obstacle_id].upper_right_point().s()
  //                        << ", t:" << path_time_obstacle_map_[obstacle_id].upper_right_point().t();

  bool status = path_time_obstacle_map_[obstacle_id].MaxMinSTofUpperLowerPoints(
    path_time_obstacle_map_[obstacle_id].lower_points(), 
    path_time_obstacle_map_[obstacle_id].upper_points());
  AINFO << "status: " << status;

  static_obs_sl_boundaries_.push_back(std::move(sl_boundary));
  AINFO << "ST-Graph mapping static obstacle: " << obstacle_id
...
std::vector<std::pair<double, double>> PathTimeGraph::GetPathBlockingIntervals(
  ...
  for (const auto& pt_obstacle : path_time_obstacles_) {
    // AINFO << "t: " << t;
    // AINFO << "pt_obstacle.min_t(): " << pt_obstacle.min_t()
    //       << ", pt_obstacle.max_t(): " << pt_obstacle.max_t();
    if (t > pt_obstacle.max_t() || t < pt_obstacle.min_t()) {
      continue;
    }

    // if obs is static, s_upper and s_lower will be all the same.
    ...
    // AINFO << "s_upper: " << s_upper << ", s_lower: " << s_lower;
...
std::vector<STPoint> PathTimeGraph::GetObstacleSurroundingPoints(
  ...
  AINFO << "GetObstacleSurroundingPoints!";
  ...
  AINFO << "num_sections: " << num_sections;
  AINFO << "t_internal: " << t_interval;
...
std::vector<std::pair<double, double>> PathTimeGraph::GetLateralBounds(
  ...
  for (size_t i = 0; i < num_bound; ++i) {
    ...
    AINFO << "ego_d_lower: " << ego_d_lower << ", ego_d_upper: " << ego_d_upper;
    // 0.1
    bounds.emplace_back(
  ...
  AINFO << "static_obs_sl_boundaries_.size(): "
        << static_obs_sl_boundaries_.size();
...
void PathTimeGraph::UpdateLateralBoundsByObstacle(
  ...
  AINFO << "UpdateLateralBoundsByObstacle!";
  AINFO << "sl_boundary.start_s(): " << sl_boundary.start_s() << ", s_end: " << s_end
        << ", sl_boundary.end_s(): " << sl_boundary.end_s() << ", s_start: " << s_start;
