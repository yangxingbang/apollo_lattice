...
class LaneFollowStage : public Stage {
 public:
  ...
  void PlanControlTestTrajectory(
      const common::TrajectoryPoint& planning_start_point, Frame* frame,
      ReferenceLineInfo* reference_line_info);  
  ...
  void GenerateControlTestTPathProfile(const ReferenceLineInfo* reference_line_info,
                                   const Frame* frame,
                                   PathData* path_data);
  ...
  void UpdataTrailerTheta(DiscretizedPath& path, const double init_trailer_theta);
