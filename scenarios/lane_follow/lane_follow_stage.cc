...
Status LaneFollowStage::PlanOnReferenceLine(
  ...
  bool enable_control_test_mode = false;
  if (enable_control_test_mode) {
    PlanControlTestTrajectory(planning_start_point, frame, reference_line_info);
  }
...
void LaneFollowStage::PlanFallbackTrajectory(
  ...
  if (reference_line_info->path_data().Empty()) {
    ...
    GenerateFallbackPathProfile(reference_line_info, frame,
...
void LaneFollowStage::PlanControlTestTrajectory(
    const TrajectoryPoint& planning_start_point, Frame* frame,
    ReferenceLineInfo* reference_line_info) {
  
  reference_line_info->mutable_path_data()->Clear();
  if (reference_line_info->path_data().Empty()) {
    AERROR << "Path fallback due to algorithm failure";
    GenerateControlTestTPathProfile(reference_line_info, frame,
                                reference_line_info->mutable_path_data());
    reference_line_info->AddCost(kPathOptimizationFallbackCost);
    reference_line_info->set_trajectory_type(ADCTrajectory::PATH_FALLBACK);
  }
  ...
  AERROR << "Speed fallback due to algorithm failure";
  *reference_line_info->mutable_speed_data() =
      SpeedProfileGenerator::GenerateConstantSpeedProfile(16.0);
...
void LaneFollowStage::GenerateFallbackPathProfile(
    const ReferenceLineInfo* reference_line_info, const Frame* frame, PathData* path_data) {
  ...
  if (!reference_line.XYToSL(adc_point.path_point(), &adc_point_s_l)) {
    ...
    DiscretizedPath path = DiscretizedPath(std::move(path_points));
    const double init_trailer_theta = frame->PlanningStartPoint().path_point().theta_trailer();
    UpdataTrailerTheta(path, init_trailer_theta);
...
void LaneFollowStage::GenerateControlTestTPathProfile(
    const ReferenceLineInfo* reference_line_info, const Frame* frame, PathData* path_data) {
  const double unit_s = 1.0;
  const auto& reference_line = reference_line_info->reference_line();
  ...
  common::math::Vec2d adc_point_xy(vehicle_state.x(), vehicle_state.y());
  common::SLPoint adc_point_s_l;

  reference_line.XYToSL(adc_point_xy, &adc_point_s_l);

  // Generate a fallback path along the reference line direction
  const auto adc_s = adc_point_s_l.s();

  std::vector<common::PathPoint> path_points;
  const double max_s = std::min(reference_line.Length(), adc_s + 120.0);
  for (double s = adc_s; s < max_s; s += unit_s) {
    const auto& ref_point = reference_line.GetReferencePoint(s);
    path_points.push_back(PointFactory::ToPathPoint(
        ref_point.x(), ref_point.y(), 0.0, s - adc_s,
        ref_point.heading(), ref_point.kappa(), ref_point.dkappa()));
    ADEBUG << std::setprecision(9)
           << "Control test path: s = " << s - adc_s
           << ", x = " << ref_point.x()
           << ", y = " << ref_point.y();
  }
  path_data->SetReferenceLine(&(reference_line_info->reference_line()));
...
bool LaneFollowStage::RetrieveLastFramePathProfile(
  ...
  bool trim_success = path_data->LeftTrimWithRefS(adc_frenet_frame_point_, init_trailer_theta);
  if (!trim_success) {
...
void LaneFollowStage::UpdataTrailerTheta(
    DiscretizedPath& path, const double init_trailer_theta) {
  std::vector<common::PathPoint> path_points;
  common::VehicleState pre_vehicle_state;
  int index = 0;
  double last_trailer_theta = init_trailer_theta;
  for (auto& point : path) {
    double theta_trailer = init_trailer_theta;
    common::VehicleState vehicle_state;
    vehicle_state.set_x(point.x());
    vehicle_state.set_y(point.y());
    vehicle_state.set_heading(point.theta());
    if (index > 0) {
      theta_trailer = TrailerStateProvider::GetTrailerTheta(
          pre_vehicle_state, vehicle_state, last_trailer_theta);
    }
    ++index;
    pre_vehicle_state = vehicle_state;
    last_trailer_theta = theta_trailer;
    point.set_theta_trailer(theta_trailer);
  }
}
