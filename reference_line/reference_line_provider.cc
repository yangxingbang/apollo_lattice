...
void ReferenceLineProvider::UpdateReferenceLine(
  ...
  if (reference_lines.size() != route_segments.size()) {
    AINFO << "The calculated reference line size(" << reference_lines.size()
...
bool ReferenceLineProvider::GetReferenceLines(
  ...
  AINFO << "Use reference line from history!";
  return true;
}
...
bool ReferenceLineProvider::CreateRouteSegments(
    ...
    std::list<hdmap::RouteSegments> *segments) {
  {
    ...
    if (!pnc_map_->GetRouteSegments(vehicle_state, segments)) {
      ...
      AERROR << "Clear reference lines history!";
      return false;
    }
    ADEBUG << "GetRouteSegments, segments.size() = " << segments->size();
  }
...
bool ReferenceLineProvider::CreateReferenceLine(
  ...
  bool is_new_routing = false;
  {
    ...
    if (pnc_map_->IsNewRouting(routing)) {
      ...
      if (!pnc_map_->UpdateRoutingResponse(routing)) {
        AINFO << "Failed to update routing in pnc map";
        return false;
      }
    }
  }
  ...
  if (!CreateRouteSegments(vehicle_state, segments)) {
    AINFO << "Failed to create reference line from routing";
    return false;
  }
  if (is_new_routing || !FLAGS_enable_reference_line_stitching) {
    Vec2d vec2d(vehicle_state.x(), vehicle_state.y());  //jch
    for (auto iter = segments->begin(); iter != segments->end();) {
      ...
      if (!SmoothRouteSegment(*iter, &reference_lines->back())) {
        ...
        common::SLPoint sl;
        if (!reference_lines->back().XYToSLForVehicleState(vec2d, &sl)) {   //jch
          AINFO << "Failed to project point: {" << vehicle_state.x() << ","
    ...
    for (auto iter = segments->begin(); iter != segments->end();) {
      ...
      if (!ExtendReferenceLine(vehicle_state, &(*iter),
        ...
        AINFO << "Failed to extend reference line";
        reference_lines->pop_back();
        iter = segments->erase(iter);
      } else {
        ++iter;
...
bool ReferenceLineProvider::ExtendReferenceLine(const VehicleState &state,
  ...
  *segments = shifted_segments;
  segments->SetProperties(segment_properties);
  common::SLPoint sl;
  if (!reference_line->XYToSLForVehicleState(vec2d, &sl)) {
...
bool ReferenceLineProvider::SmoothRouteSegment(const RouteSegments &segments,
  ...
  if(FLAGS_print_log_parse_reference_line){
    AINFO << "log-parse::reference line map points(x,y): " << std::fixed << std::setprecision(2) << 0.0 <<  " , " << 0.0;
    for (auto path_point : path.path_points()) {
      AINFO << "log-parse::reference line map points(x,y): " << std::fixed << std::setprecision(2) 
        << path_point.x() <<  " , " << path_point.y();
    }
  }
  bool smooth_state = SmoothReferenceLine(ReferenceLine(path), reference_line); //将path上的点拷贝到reference_points点上，完成reference_line的初始化， 然后进行参考线平滑
  if(FLAGS_print_log_parse_reference_line){
    AINFO << "log-parse::reference line smoothed points(x,y): " << std::fixed << std::setprecision(2) << 0.0 <<  " , " << 0.0;
    for (auto smooth_point : reference_line->reference_points()) {
      AINFO << "log-parse::reference line smoothed points(x,y): " << std::fixed << std::setprecision(2) 
