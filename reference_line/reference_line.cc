...
bool ReferenceLine::XYToSLForVehicleState(const common::math::Vec2d& xy_point,  //jch
                           SLPoint* const sl_point) const {
  double s = 0.0;
  double l = 0.0;
  auto vehicle_state = VehicleStateProvider::Instance();
  AINFO << "enter XYToSLForVehicleState" << "; vehicle_state->heading(): " << vehicle_state->heading();
  if (!map_path_.GetProjectionWithHeading(xy_point, &s, &l, vehicle_state->heading())) {
    AERROR << "Cannot get nearest point from path.";
    return false;
  }
  sl_point->set_s(s);
...
double ReferenceLine::GetSpeedLimitFromS(const double s) const {
  for (const auto& speed_limit : speed_limit_) {
    ADEBUG << "speed_limit.start_s = " << speed_limit.start_s 
           << ", end_s = " << speed_limit.end_s
  ...
  if (!FLAGS_enable_map_speed_limit) {
    return speed_limit;
  }
  for (const auto& lane_waypoint : map_path_point.lane_waypoints()) {
    ...
    // 限速暂时处理    added by zhangyf 
    if (lane_waypoint.lane->road_id().id() == "1000003" || 
        lane_waypoint.lane->road_id().id() == "1016018" ||
        lane_waypoint.lane->road_id().id() == "1048010" || 
        lane_waypoint.lane->road_id().id() == "1016013" ||
        lane_waypoint.lane->road_id().id() == "1016021" || 
        lane_waypoint.lane->road_id().id() == "1000006" ||
        lane_waypoint.lane->road_id().id() == "1000007" ) {
      speed_limit = std::fmin(lane_waypoint.lane->lane().speed_limit(), 11.1);
    }
  }
  ADEBUG << "speed_limit from lane_waypoint: " << speed_limit;
  return speed_limit;
}
