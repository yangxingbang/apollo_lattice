...
Status OnLanePlanning::InitFrame(const uint32_t sequence_num,
  ...
  if (!status.ok()) {
    AINFO << "failed to init frame:" << status.ToString();
    return status;
  }
  return Status::OK();
}
...
void OnLanePlanning::RunOnce(const LocalView& local_view,
  ...
  AINFO << std::setprecision(16)
        << "RunOnce start_timestamp: " << start_timestamp
        << ", vehicle_state_timestamp: " << vehicle_state_timestamp
        << ", localization.measurement_time: " 
        << local_view_.localization_estimate->measurement_time();
  if (start_timestamp < vehicle_state_timestamp) {
    AINFO << "ERROR, start_timestamp is behind vehicle_state_timestamp by "
  ...
  TractorTrailerModel::UpdateTrailerYawAngle(vehicle_state);
  ...
  for (auto& ref_line_info : *frame_->mutable_reference_line_info()) {
    ...
    if(FLAGS_print_log_parse_reference_line){
      AINFO << "log-parse::reference line final points(x,y): " << std::fixed << std::setprecision(2) << 0.0 <<  " , " << 0.0;
      for (auto final_point : ref_line_info.reference_line().reference_points()) {
        AINFO << "log-parse::reference line final points(x,y): " << std::fixed << std::setprecision(2) 
...
bool OnLanePlanning::CheckPlanningConfig(const PlanningConfig& config) {
  ...
  //   return false;
  // }
  if (!config.standard_planning_config().has_planner_lattice_config()) {
    return false;
  }
